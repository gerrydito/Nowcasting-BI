...
```{r}
tk_augment_differences(.data=df,
                        .value = ADHK,
                        .differences = 1) %>% 
...
```{r}
#| code-fold: true
cross_validate_lstm <- function(data,
                                date_col,
                                input_cols,
                                output_cols,
                                initial,
                                assess,
                                skip,
                                metric_fun = yardstick::rmse,
                                ...) {
  date_col <- rlang::ensym(date_col)

  # 1) generate rolling splits
  ...
  # 2) fit + forecast on each split
  preds_list <- purrr::map(splits$splits, function(split) {
    train_data <- rsample::analysis(split)
    test_data  <- rsample::assessment(split)

    # train on analysis
    history <- train_lstm_mimo(
      data        = train_data,
      date_col    = !!date_col,
      input_cols  = input_cols,
      output_cols = output_cols,
    ...
    # forecast on assessment
    pred_tbl <- predict_lstm(history, train_data %>% slice_tail(n=1))

    # align actuals (rename date → date so join works)
    actual_tbl <- test_data %>%
      select(!!date_col, all_of(output_cols)) %>%
      rename(date = !!date_col)

    # pivot if multiple output_cols
    full_join(actual_tbl, pred_tbl, by = "date")
  })

  # 3) compute metrics per split
  metrics_tbl <- purrr::imap_dfr(preds_list, function(df, i) {
    # df has columns: date, <output_cols> (actual), <output_cols> (forecast)
    # assume same names for actual and forecast columns
    # we rename forecasts to .pred internally

    df_long <- df %>%
      rename_with(~ ".pred", all_of(output_cols))

    res <- metric_fun(
      df_long,
      truth    = !!rlang::sym(output_cols),
      estimate = .pred
    )
    res$id <- splits$id[i]
    res
  ...
  # 4) combine predictions into one tibble
  preds_tbl <- purrr::imap_dfr(preds_list, function(df, i) {
    df$id <- splits$id[i]
    df
  })
...
plot_cv_forecasts <- function(cv_res, split = NULL) {
  df <- cv_res$predictions
  if (!is.null(split)) {
    df <- filter(df, id == split)
  }
  ...
  ggplot(df_long, aes(x = date, y = value, color = series)) +
    geom_line() +
    labs(
      title = ifelse(is.null(split),
                     "All Splits: Actual vs Forecast",
                     paste0("Split ", split, ": Actual vs Forecast")),
      x     = "Date",
      y     = "Value",
      color = NULL
    ) +
...
```{r}
cv_mod <- cross_validate_lstm(data = train_df,
                     input_cols = input_cols,
                     output_cols = output_cols,
                     date_col = "date_lg0",
                     val_split = 0.2,
                     epochs = 1000,
                     batch_size = nrow(train_df),
                     optimizer = "adam",
                     hidden_size = 200,
                     num_layers = 1,
                     activation = "linear",
